...
"""
            case TaskName.KNOWLEDGE_GRAPH:
                requirement_str = """- Review both the past experience and the current question.
- Identify the knowledge-graph operations required for both the past experience and the current question. For example, relevant operations include: "get_relations", "get_neighbors", "intersection", "get_attributes", "argmax", "argmin", "count", and finishing with a variable answer.
- Determine if the current question shares similar entities, relations, or operation patterns with the past experience.
"""
            case _:
...
Now, I will give you the question that you need to solve."""
    ...
    def _extract_action(self, agent_raw_inference: str) -> Optional[str]:
        match self.task_name:
            ...
            case TaskName.KNOWLEDGE_GRAPH:
                try:
                    knowledge_graph_parser_result = (
                        KnowledgeGraph._parse_agent_response(agent_raw_inference)
                    )
                except:  # noqa
                    return None
                match knowledge_graph_parser_result.action:
                    case AgentAction.EXECUTE:
                        action_content = knowledge_graph_parser_result.content or ""
                        action_content = re.sub(r"\s*,\s*", ", ", action_content)
                        action_content = re.sub(r"\(\s*", "(", action_content)
                        action_content = re.sub(r"\s*\)", ")", action_content)
                        return f"Action: {action_content}"
                    case AgentAction.FINISH:
                        final_answer = knowledge_graph_parser_result.content or ""
