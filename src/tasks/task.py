...
class Task(TaskInterface, Generic[DatasetItemSubclass]):
    ...
    @final
    def get_dataset_item_for_sample(self, sample_index: SampleIndex) -> DatasetItemSubclass:
        """
        Read-only access to a dataset item before `reset()` binds it as the
        current sample. This is useful for controllers that need to choose a
        policy before prompt construction (e.g. replay-gated routing).
        """
