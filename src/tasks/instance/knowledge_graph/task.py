...
class KnowledgeGraph(Task[KnowledgeGraphDatasetItem]):
    ...
    def __init__(
        ...
        data_file_path: str,
        max_round: int,
        strict_protocol_prompt: bool = False,
    ):
        super().__init__(task_name, chat_history_item_factory, max_round)
        self.strict_protocol_prompt = strict_protocol_prompt
        ...
        for key, item in raw_dataset.items():
            ...
            entity_dict = self._maybe_parse_literal(item["entity_dict"])
            answer_list = self._maybe_parse_literal(item["answer_list"])
            if not isinstance(entity_dict, dict):
                raise TypeError(
                    f"Expected entity_dict to be dict-like for sample {key}, got {type(entity_dict).__name__}"
                )
            if not isinstance(answer_list, Sequence) or isinstance(answer_list, str):
                raise TypeError(
                    f"Expected answer_list to be sequence-like for sample {key}, got {type(answer_list).__name__}"
                )
    ...
    def _extract_argument_list_from_argument_str(
        ...
        if len(argument_str) == 0:
            return []
        # Some chat models render tool arguments as Python-style string
        # literals. LAB's KG grammar uses raw entity/relation tokens, so
        # normalize only matching outer quotes before validating arguments.
        for entity in sorted(entity_list, key=len, reverse=True):
            argument_str = argument_str.replace(f'"{entity}"', entity)
            argument_str = argument_str.replace(f"'{entity}'", entity)
    ...
    def _reset(self, session: Session) -> None:
        ...
        question_content = f"Question: {question}, Entities: {entity_list}"
        if self.strict_protocol_prompt:
            question_content += (
                "\n\nStrict protocol reminder: use raw, unquoted arguments "
                "(for example, Action: get_relations(Havarti), not "
                'Action: get_relations("Havarti")). Use only an entity '
                "listed above or an existing #variable. A literal entity "
                "name is never a valid final response. Finish only with "
                "Final Answer: #N, where #N is a variable created by a "
                "previous action. Do not repeat an action that already "
                "returned an error or the same relations."
            )
        session.chat_history.inject(
            {
                "role": Role.USER,
